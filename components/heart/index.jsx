...
const animationClip = new THREE.AnimationClip(null, scaleFn(1), [
  new THREE.NumberKeyframeTrack('.time', [0, 0.20, 0.40, 0.60, 0.80, 1].map(scaleFn), [1, 1.2, 1.4, 1.5, 1.3, 1].map(scaleFn), THREE.InterpolateSmooth),
])
...
const generateColor = (address) => {
  ...
  const saturation = 1 + (((100.0 - satPercent) * startSaturation + satPercent * endSaturation) / 100);
  const lightPercent = easeInQuint(10);
  const lightness = 1 + (((100.0 - lightPercent) * startLightness + (lightPercent * endLightness)) / 100);
...
const Heart = ({ address, record, attributes }) => {  
  ...
  const colorContrast = useMemo(() => lerpColor('#262626', '#f3f4f6', attributes.ethereumActivity), [attributes.ethereumActivity])
  const interpolatedAttributes = useMemo(() => Object.keys(interpolators).reduce((acc, key) => {
