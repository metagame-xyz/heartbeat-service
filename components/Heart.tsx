...
class BlobMaterial extends THREE.ShaderMaterial {
    constructor() {
      ...
      void main() {
        ...
        color = mix(color, rgb(255.0, 154.0, 0.0), noise2 * uNetwork2);
...
const Model = ({ color, colorContrast, loop, ...props }) => {
...
export const BaseHeart = (props) => {
    return (
        <Canvas dpr={[1, 2]} camera={{ fov: 60, position: [-3, 2, -3] }}>
            <Model {...props} />
